...
export interface ModelConfig {
    provider:
        "ollama" | "groq" | "claude" | "openai" | "openrouter" | "builtin-ai" | "custom-openai";
    model: string;
    whisperModel: string;
    apiKey?: string | null;
            ...
            try {
                ...
                if (data && data.provider !== null) {
                    ...
                    if (data.provider === "custom-openai") {
                        try {
                            const customConfig = await invoke<CustomOpenAIConfig>(
        ...
        try {
            ...
            if (data.length > 0 && !modelConfig.model) {
                const firstAvailable = data.find(
                    (m: BuiltInModelInfo) => m.status?.type === "available"
                );
                if (firstAvailable) {
    ...
    const handleSave = async () => {
        ...
        const updatedConfig = {
            ...
            customOpenAIEndpoint:
                modelConfig.provider === "custom-openai" ? customOpenAIEndpoint.trim() : null,
            customOpenAIModel:
                modelConfig.provider === "custom-openai" ? customOpenAIModel.trim() : null,
            customOpenAIApiKey:
                        ...
                        <Select
                            ...
                            onValueChange={(value) => {
                                ...
                                if (provider === "custom-openai") {
                                    invoke<CustomOpenAIConfig>("api_get_custom_openai_config")
                                        .then((config) => {
                                            if (config) {
                                        ...
                                        .catch((err) => {
                                            logger.error(
                                                "Failed to load custom OpenAI config:",
                                                err
                                            );
                                        });
                                }
                                    ...
                                    <div className="space-y-4">
                                        <Alert className="border-orange-500 bg-orange-50">
                                            <AlertDescription className="text-orange-800">
                                                Ollama is not installed or not running. Please
                                                download and install Ollama to use local models.
                                            </AlertDescription>
                                        </Alert>
                                        <Button
                                            variant="default"
