...
const [vocabActive, setVocabActive] = useState(false);
    ...
    // Check if custom vocabulary is active
    useEffect(() => {
        if (!open) return;
        invoke<string>("api_get_custom_vocabulary")
            .then((raw) => setVocabActive(!!raw && raw.trim().length > 0))
            .catch(() => setVocabActive(false));
    }, [open]);
                    ...
                    {!isProcessing &&
                        ...
                        (!isParakeetModel ? (
                            <div className="space-y-3">
                                ...
                                {vocabActive && (
                                    <p className="text-xs text-blue-600 flex items-center gap-1">
                                        ...
                                        Custom vocabulary library is active
                                    </p>
                                )}
                            </div>
                        ) : (
                            <div className="space-y-3">
                                ...
                                <p className="text-xs text-muted-foreground">
                                    Language selection isn't supported for Parakeet. It always uses
                                    automatic detection.
                                </p>
                            </div>
                        ))}
