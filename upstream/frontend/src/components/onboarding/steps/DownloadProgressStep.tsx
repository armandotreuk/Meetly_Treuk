...
export function DownloadProgressStep() {
    ...
    const handleRetryDownload = async () => {
        ...
        if (retryingRef.current) {
            logger.debug("[DownloadProgressStep] Retry already in progress, ignoring");
            return;
        }

        logger.debug("[DownloadProgressStep] Retrying Parakeet download");
    ...
    const handleRetrySummaryDownload = async () => {
        ...
        if (retryingSummaryRef.current) {
            logger.debug("[DownloadProgressStep] Summary retry already in progress, ignoring");
            return;
        }

        logger.debug("[DownloadProgressStep] Retrying summary model download");
    ...
    const handleContinue = async () => {
        ...
        try {
            ...
            if (actuallyAvailable && !parakeetDownloaded) {
                logger.debug("[DownloadProgressStep] Model available but state not updated");
                setParakeetDownloaded(true);
                setParakeetState((prev) => ({
                    ...prev,
                    status: "completed",
                ...
                <div className="space-y-2">
                    ...
                    <div className="flex items-center justify-between text-sm">
                        <span className="text-gray-600">
                            {state.downloadedMb.toFixed(1)} {sizeUnit} / {state.totalMb.toFixed(1)}{" "}
                            {sizeUnit}
                        </span>
