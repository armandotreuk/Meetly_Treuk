            ...
            try {
                ...
                const unlistenProgress = await listen<{ modelName: string; progress: number }>(
                    ...
                    (event) => {
                        ...
                        logger.debug(
                            `🔵 [OllamaDownloadContext] Progress for ${modelName}: ${progress}%`
                        );
                ...
                const unlistenComplete = await listen<{ modelName: string }>(
                    ...
                    (event) => {
                        ...
                        logger.debug(
                            `✅ [OllamaDownloadContext] Download complete for ${modelName}`
                        );
                ...
                const unlistenError = await listen<{ modelName: string; error: string }>(
                    ...
                    (event) => {
                        ...
                        logger.error(
                            `❌ [OllamaDownloadContext] Download error for ${modelName}:`,
                            error
                        );
