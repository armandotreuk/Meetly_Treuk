                ...
                if (data && data.provider) {
                    ...
                    if (data.provider === "custom-openai") {
                        try {
                            ...
                            logger.error(
                                "[ConfigContext] Failed to fetch custom OpenAI config:",
                                err
                            );
                        }
                    }
        ...
        const loadAllApiKeys = async () => {
            try {
                ...
                const keys = await Promise.all(
                    providers.map(
                        (p) => invoke<string>("api_get_api_key", { provider: p }).catch(() => null) // Gracefully handle missing keys
                    )
                );
                ...
                logger.debug("[ConfigContext] Loaded provider API keys");
            } catch (error) {
