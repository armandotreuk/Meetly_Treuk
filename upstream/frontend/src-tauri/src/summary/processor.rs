...
/// Append the user's in-meeting personal notes to the final report user prompt as an
/// additional important source. The LLM is instructed to incorporate, improve and expand
/// upon them. Empty/whitespace-only notes (or `None`) leave the prompt untouched.
pub(crate) fn append_user_notes_prompt(prompt: &mut String, notes: Option<&str>) {
    let Some(notes) = notes.map(|n| n.trim()).filter(|n| !n.is_empty()) else {
        return;
    };
    prompt.push_str("\n\nParticipant's personal notes taken during the meeting — treat as an important additional source. Incorporate, improve and expand upon them in the summary:\n\n<user_notes>\n");
...
pub async fn generate_meeting_summary(
        ...
        resolve_cached_english(cached_english, summary_language)
    {
        ...
        append_user_notes_prompt(&mut final_user_prompt, user_notes);

        // Append the global custom vocabulary so the LLM uses the user's exact terminology
        let vocab_prompt = crate::get_vocabulary_prompt_internal();
        if !vocab_prompt.is_empty() {
            final_user_prompt.push_str("\n\nGlossary — use this exact terminology: ");
            final_user_prompt.push_str(&vocab_prompt);
        }
...
mod tests {
    ...
    #[test]
    fn append_user_notes_prompt_some_appends_tagged_block() {
        let mut prompt = String::from("<transcript_chunks>\nbody\n</transcript_chunks>");
        append_user_notes_prompt(&mut prompt, Some("Action: follow up with vendor"));
        assert!(prompt.contains("<user_notes>\nAction: follow up with vendor\n</user_notes>"));
        assert!(prompt.contains("important additional source"));
    }

    #[test]
    fn append_user_notes_prompt_none_or_empty_is_noop() {
        let original = String::from("<transcript_chunks>\nbody\n</transcript_chunks>");

        let mut p_none = original.clone();
        append_user_notes_prompt(&mut p_none, None);
        assert_eq!(p_none, original);
