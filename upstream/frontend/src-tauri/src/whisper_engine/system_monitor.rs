...
impl SystemMonitor {
    ...
    async fn get_cpu_temperature(&self, _system: &System) -> Option<f32> {
        ...
        // Temperature sensors are unreliable across platforms (and require
        // elevated permissions on some OSes), so we return None by design.
        // The whisper monitor only needs CPU/RAM which are already reported.
        None
    }
