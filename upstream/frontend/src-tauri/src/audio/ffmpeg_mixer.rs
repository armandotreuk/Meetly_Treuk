...
impl TimestampedChunk {
    fn new(samples: Vec<f32>, _sample_rate: u32) -> Self {
        Self {
            samples,
...
pub struct FFmpegAudioMixer {
    ...
    mixer: AudioMixer,
    // Stored for introspection (see `test_ffmpeg_mixer_creation`) and so external
    // consumers that later need to reason about window sizes can read it without
    // recomputing from constructor arguments. Not read by the mixing loop itself.
    #[allow(dead_code)]
    sample_rate: u32,
