...
pub async fn start_recording_with_meeting_name<R: Runtime>(
    ...
    // Emit success event
    // ponytail: same reasoning as in start_recording_with_devices_and_meeting — the recording
    // is already running, so a failed notification emit must not surface as a "Recording Failed"
    // alert in the UI.
...
pub async fn start_recording_with_devices_and_meeting<R: Runtime>(
    ...
    // Emit success event
    // ponytail: the recording is already running at this point (pipeline + streams started,
    // IS_RECORDING=true, transcription task spawned). The emit is a notification, not a gate —
    // propagating its failure with `?` would make the frontend show "Recording Failed" while
    // audio is actually being captured. Log and continue instead.
    if let Err(e) = app.emit(
        "recording-started",
        serde_json::json!({
...
pub async fn stop_recording<R: Runtime>(
    ...
    // Step 3.5: Analytics stripped (decision 3: no telemetry)
