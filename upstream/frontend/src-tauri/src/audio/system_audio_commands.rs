...
pub async fn start_system_audio_capture_command() -> Result<String, String> {
    match start_system_audio_capture().await {
        Ok(_stream) => {
            // Stream is owned by the audio pipeline; cancellation goes
            // through stop_system_audio_capture, so no extra global handle needed.
    ...
    async fn test_list_system_audio_devices() {
        ...
        match devices {
            Ok(device_list) => {
                ...
                let _ = device_list.len(); // Should at least not crash
            }
            Err(e) => {
